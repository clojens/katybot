...
(defn- user-from-campfire
  [user]
  (change-keys user :avatar_url :avatar))

(defn type-from-campfire
  [type]
  (case type
    "TextMessage"  :text
    "EnterMessage" :join
...
(defn +campfire-receptor
  [robot account room token]
  (assoc robot
    :receptor ::campfire-receptor
    ::api     (api/campfire-async-api account token)
    ::room    room))

(defmethod listen ::campfire-receptor
  [{api ::api, room ::room  :as  robot}]
  (let [me (api/user-me api)]
    (api/listen api room (partial msg-callback robot (:id me)))
    (api/say    api room "Hi everybody!"))
  :listening)

(defmethod say ::campfire-receptor
  [{api ::api, room ::room} msg]
  (api/say api room msg))

(defmethod say-img ::campfire-receptor
  [robot url]
  (say robot [url "#.png"]))

(defmethod user ::campfire-receptor
  [{api ::api} user-id]
  (user-from-campfire (api/user-info api user-id)))

(defmethod users ::campfire-receptor
  [{api ::api, room ::room}]
...
(defmethod shutdown ::campfire-receptor
  [{api ::api, room ::room}]
